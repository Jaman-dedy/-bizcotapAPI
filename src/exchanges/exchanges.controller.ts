import {
  ...
  import { CreateExchangeDto, ExchangeResponseDto, ExchangeQueryDto } from './dto';
  ...
  export class ExchangesController {
    ...
    @Public()
    @Post('contacts')
    @ApiOperation({ summary: 'Record a contact exchange when someone scans a tag' })
    @ApiResponse({ status: 201, description: 'Exchange recorded successfully', type: ExchangeResponseDto })
    ...
    @Get('contacts')
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Get all contact exchanges for the authenticated user' })
    @ApiResponse({ status: 200, description: 'List of exchanges', type: [ExchangeResponseDto] })
    async findAllExchanges(@Request() req: any, @Query() query: ExchangeQueryDto) {
    ...
    @Get('statistics')
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Get exchange statistics for the authenticated user' })
